...
class ThermalCam:
    ...
    def show_live_preview(self):
        ...
        try:
            ...
            cmd = [
                ...
                "/tmp/preview.jpg",
                "--width",
                "256",  # Slightly higher res for better quality
                "--height",
                "192",
                "-t",
                "0",  # Immediate capture
    ...
    def add_caption(self, img, text):
        ...
        for adj_x in range(-outline_width, outline_width + 1):
            for adj_y in range(-outline_width, outline_width + 1):
                draw.text(
                    (x_watermark + adj_x, y_watermark + adj_y),
                    watermark_text,
                    font=font,
                    fill="black",
                )
        ...
        for adj_x in range(-outline_width, outline_width + 1):
            for adj_y in range(-outline_width, outline_width + 1):
                draw.text(
                    (x_date + adj_x, y_date + adj_y), text, font=font, fill="black"
                )
    ...
    def apply_ordered_dithering(self, img):
        ...
        threshold_map = [[0, 8, 2, 10], [12, 4, 14, 6], [3, 11, 1, 9], [15, 7, 13, 5]]
        ...
        for y in range(height):
            for x in range(width):
                ...
                old_pixel = (
                    int(pixels[x, y])
                    if isinstance(pixels[x, y], (int, float))
                    else pixels[x, y]
                )
    ...
    def call_api(self, pil_image, mode):
        ...
        is_search = bool(result.get("title") or result.get("price"))

        # Handle not_found (e.g. pricing couldn't identify the item)
        if status == "not_found" or not result.get("image_b64"):
            return None, text, is_search

        # Decode the returned image
        img_bytes = base64.b64decode(result["image_b64"])
        api_img = Image.open(io.BytesIO(img_bytes))

        return api_img, text, is_search
    ...
    def print_image(self, img):
        ...
        self.printer.image(
            img, high_density_vertical=True, high_density_horizontal=True
        )
        self.printer.text("\n\n\n")
        ...
        try:
            ...
            if current_mode != "Standard":
                try:
                    ...
                    api_img, api_text, is_search = self.call_api(
                        raw_img_rotated, current_mode
                    )

                    if api_img is None:
                        # Search mode returned not_found — fall back to raw image
                        print(f"API returned not_found: {api_text}")
                        self.display_status("NOT FOUND", large=True)
                        time.sleep(1)
                    else:
            ...
            aspect_ratio = final_img.height / final_img.width
            new_height = int(PRINTER_WIDTH * aspect_ratio)
            final_img_resized = final_img.resize(
                (PRINTER_WIDTH, new_height), Image.Resampling.LANCZOS
            )

            # Skip caption for search modes — the returned image already has text baked in
            if is_search:
                final_img_with_text = final_img_resized
            else:
